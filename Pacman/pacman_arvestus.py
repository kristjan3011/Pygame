"""
PAC-MAN arvestustoo versioon.

Run:
    python pacman_arvestus.py

Controls:
    Menu: 1 = classic mode, 2 = beginner mode
    Game: arrow keys or WASD = move, P = pause, R = restart,
          M = menu, ESC = quit
"""
...
# Basic display and grid settings.
CELL = 24
COLS = 21
ROWS = 23
W = COLS * CELL
...
# Classic map is based on pacman1.py.
RAW_MAP_CLASSIC = [
...
# Beginner map is inspired by the compact beginner version in pacman2.py,
# but it is converted to the same 21 x 23 grid system as the classic mode.
RAW_MAP_BEGINNER = [
    "WWWWWWWWWWWWWWWWWWWWW",
    "W.........W.........W",
    "W.WWW.WWW.W.WWW.WWW.W",
...
GHOST_HOMES = [
    (9 * CELL + CELL // 2, 9 * CELL + CELL // 2),
    (10 * CELL + CELL // 2, 9 * CELL + CELL // 2),
    (11 * CELL + CELL // 2, 9 * CELL + CELL // 2),
    (12 * CELL + CELL // 2, 9 * CELL + CELL // 2),
]
GHOST_EXIT_CELL = (10, 11)
...
MODES = {
    "classic": {
        ...
        "ghost_ai": "bfs",
        "player_start": (10, 17),
        "info": "Original-style maze, 4 ghosts and faster chase.",
    },
    "beginner": {
        "title": "Beginner Mode",
        ...
        "ghost_ai": "random",
        "player_start": (10, 19),
        "info": "Simpler maze, 2 slower random-walk ghosts.",
    },
}


def build_map(raw_map):
    """Convert a text map into a 2-D tile grid."""
    grid = []
...
def is_tunnel_row(grid, gy):
    """Return True when a row has an open left or right side."""
    if not 0 <= gy < ROWS:
        return False
...
class Pacman:
    """Player character: handles movement, collision and mouth animation."""
...
class Ghost:
    """Enemy character with mode-based AI and shared drawing/collision logic."""

    FRIGHT_TIME = 8 * FPS

    def __init__(self, grid, gid, home, speed):
        self.grid = grid
        self.gid = gid
        self.home = home
        self.speed = speed
    ...
    def _blocked_pixel(self, x, y):
        ...
        # Ghosts may pass through the gate; Pacman may not.
    ...
    def update(self, pacman):
        ...
        # Beginner mode uses random walk, so ghosts do not target Pacman with BFS.
        if self.ai_mode == "random":
            if self._at_cell_center():
                self.dir = self._choose_random_dir()
            self._move(self.dir)
...
def ghost_exit_delay(gid):
    """Stagger ghost starts so all enemies do not leave the house at once."""
    return [0, 60, 120, 200][gid]


class BonusFruit:
    """Temporary bonus item that appears after enough dots are collected."""
...
class Game:
    """Main game controller: state, modes, input, scoring and drawing."""

    def __init__(self):
        pygame.init()
        self.screen = pygame.display.set_mode((W, H))
        pygame.display.set_caption("PAC-MAN arvestustoo")
    ...
    def update_high_score(self):
        """Track the best score only during the current program run."""
    ...
    def new_game(self, mode=None):
        ...
        for i in range(config["ghost_count"]):
            ghost = Ghost(self.grid, i, GHOST_HOMES[i], config["ghost_speed"])
    ...
    def update_bonus_fruit(self):
        # The fruit appears once at two progress points per level.
    ...
    def draw(self):
        ...
        if self.state == "paused":
            ...
            self.draw_overlay("YOU LOST A LIFE", f"Lives left: {self.lives}")
        elif self.state == "win":
            self.draw_overlay(f"LEVEL {self.level} CLEAR", "Next level starts soon")
